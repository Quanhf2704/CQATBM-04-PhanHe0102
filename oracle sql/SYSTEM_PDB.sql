...
-- AUDIT
--- Kích hoạt ghi nhật ký hệ thống (System Audit)
-- Đăng nhập bằng quyền sys vào database CDB$ROOT
ALTER SESSION SET CONTAINER = CDB$ROOT;

--- Kiểm tra trạng thái Unified Auditing:
SELECT value FROM v$option WHERE parameter = 'Unified Auditing';
--SHOW PARAMETER AUDIT;

--- Cài đặt tham số AUDIT_TRAIL
ALTER SYSTEM SET AUDIT_TRAIL='DB' SCOPE=SPFILE;
--ALTER SYSTEM SET AUDIT_TRAIL='DB,EXTENDED' SCOPE=SPFILE;

--- Khởi động lại Oracle Database để thay đổi có hiệu lực
SHUTDOWN IMMEDIATE;
STARTUP;

--- Chuyển sang pluggable database
ALTER SESSION SET CONTAINER = XEPDB1;

-- Theo dõi hành vi của các user trên tất cả các table
AUDIT ALL ON USERDBA.DANGKY BY ACCESS;
AUDIT ALL ON USERDBA.DONVI BY ACCESS;
AUDIT ALL ON USERDBA.HOCPHAN BY ACCESS;
AUDIT ALL ON USERDBA.KHMO BY ACCESS;
AUDIT ALL ON USERDBA.NHANSU BY ACCESS;
AUDIT ALL ON USERDBA.PHANCONG BY ACCESS;
AUDIT ALL ON USERDBA.SINHVIEN BY ACCESS;
AUDIT ALL ON USERDBA.THONGBAO BY ACCESS;

-- Theo dõi hành vi của các user trên các view
AUDIT GRANT, INSERT, UPDATE,DELETE ON USERDBA.ROLE_PRIVILEGES;
AUDIT GRANT, INSERT, UPDATE,DELETE ON USERDBA.USER_PRIVILEGES;
AUDIT GRANT, INSERT, UPDATE,DELETE ON USERDBA.UV_SEL_GIANGVIEN_DANGKY BY ACCESS;
AUDIT GRANT, INSERT, UPDATE,DELETE ON USERDBA.UV_SEL_GIANGVIEN_PHANCONG BY ACCESS;
AUDIT GRANT, INSERT, UPDATE,DELETE ON USERDBA.UV_SEL_NHANVIEN BY ACCESS;
AUDIT GRANT, INSERT, UPDATE,DELETE ON USERDBA.UV_TDV_MODIFY_PHANCONG BY ACCESS;
AUDIT GRANT, INSERT, UPDATE,DELETE ON USERDBA.UV_TDV_XEM_PHANCONG_GIANGVIEN_CUA_DONVI BY ACCESS;

NOAUDIT ALL ON USERDBA.UV_SEL_NHANVIEN;
NOAUDIT ALL ON USERDBA.NHANSU;

-- Theo dõi hành vi của các user trên các Stored Procedure
AUDIT EXECUTE ON USERDBA.CREATE_USER BY ACCESS;
AUDIT EXECUTE ON USERDBA.DROP_USER BY ACCESS;
AUDIT EXECUTE ON USERDBA.CREATE_ROLE BY ACCESS;
AUDIT EXECUTE ON USERDBA.DROP_ROLE BY ACCESS;
AUDIT EXECUTE ON USERDBA.GRANT_PERMISSION_ROLE BY ACCESS;
AUDIT EXECUTE ON USERDBA.GRANT_PERMISSION_USER BY ACCESS;
